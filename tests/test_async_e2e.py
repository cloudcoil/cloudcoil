...
async def test_async_crd_scale_operations(test_config, wait_for_crd_discovery):
    with test_config:
        ns = await k8s.core.v1.Namespace(metadata=ObjectMeta(generate_name="test-")).async_create()
        ...
        await crd.async_wait_for(check_established, timeout=30)
        DynamicWebService = get_dynamic_resource("WebService", f"{crd.spec.group}/v1alpha1")
        await asyncio.to_thread(wait_for_crd_discovery, DynamicWebService)
